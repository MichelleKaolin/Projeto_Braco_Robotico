...
void loop() {
  if (Serial.available() > 0) {
    ...
    if (motor.isValid) {
      ...
      if (motor.motorID == 1) {
        Servo1.write(motor.motorAngle);
      } else if (motor.motorID == 2) {
        Servo2.write(motor.motorAngle);
      } else if (motor.motorID == 3) {
        Servo3.write(motor.motorAngle);
      } else if (motor.motorID == 4) {
        Servo4.write(motor.motorAngle);
      } else if (motor.motorID == 5) {
        Servo5.write(motor.motorAngle);
      } else if (motor.motorID == 6) {
        Servo6.write(motor.motorAngle);
      } else {
...
MotorData executarAcao() {
  ...
  if (dataLength == 5) {
    ...
    if (motor.motorAngle < 0 || motor.motorAngle > 220) {
